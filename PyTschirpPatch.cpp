...
PyTschirp::PyTschirp(std::shared_ptr<midikraft::Patch> p, std::weak_ptr<midikraft::Synth> synth, int layerNo) : PyTschirp(p, synth)
{
	...
	auto layeredPatch = patch_->getCapability<midikraft::LayeredPatchCapability>();
	if (layeredPatch) {
...
void PyTschirp::set_attr(std::string const &name, std::vector<int> const &value)
{
	...
	if (isChannelValid()) {
		auto liveEditing = attr.def()->getCapability<midikraft::SynthParameterLiveEditCapability>();
		if (liveEditing) {
...
{
	auto storedName = patch_->getCapability<midikraft::StoredPatchNameCapability>();
	if (patch_) {
...
std::vector<std::string> PyTschirp::parameterNames()
{
	...
	auto params = patch_->getCapability<midikraft::DetailedParametersCapability>();
	if (params) {
...
{
	if (!synth_.expired()) {
		auto location = synth_.lock()->getCapability<midikraft::MidiLocationCapability>();
		if (location) {
